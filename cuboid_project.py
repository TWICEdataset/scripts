...
def open_label_read(path,frame,list_images,obj_ego,cbla,synthetic,OTA,without):
    ...
    #intrinsic_matrix = np.array([[1942.94197430278,0,0],[0,1941.98949906699,0],[961.402331648884,587.317863907068,1]])
    ...
    def project_cuboid_image(path,frame,projectCuboid,showImage,showVideo,saveImage,showFrame,showTimestamp,pathSaveImage=""):
        ...
        if projectCuboid: 
            for i in range(len(list_images)):
                ...
                img = cv2.circle(img, (int(xyImagePoints[5][0]),int(xyImagePoints[5][1])), radius=5, color=(255, 0, 0), thickness=-1)
                ...
                #line to see delay xyImagePoints[6],xyImagePoints[5]
                #img=cv2.line(img,(0,604),(1920,604),(255,150,0),2)
        ...
        if showVideo:
            ...
            out = cv2.VideoWriter(os.path.join(pathSaveImage,name_file+".avi"),cv2.VideoWriter_fourcc(*'DIVX'), 30, size)
            for i in range(len(list_images)):
